...
import { TrendingUp, TrendingDown, Minus, Play, Heart } from "lucide-react"
import { MetricsChart } from "@/components/charts/metrics-chart"
import { exportSongMetrics } from "@/lib/export-content-metrics"
import type { SongOverviewResponse, MetricValue } from "@/types/catalog"
import { REGION_LABELS } from "@/types/catalog"
import type { DateRange } from "react-day-picker"

interface SongMetricsOverviewProps {
  metrics: SongOverviewResponse | undefined
  isLoading: boolean
...
export function SongMetricsOverview({ 
  metrics, 
  isLoading,
  onPeriodChange,
  onDateRangeChange,
  ...
  const comparisonData = [
    {
      ...
      Likes: metrics.periodLikes.previous,
    },
    {
      name: 'Current Period',
      Plays: metrics.periodPlays.current,
      Likes: metrics.periodLikes.current,
    },
  ]
    ...
    <div className="space-y-4">
      <div className="flex flex-col space-y-4">
        <div className="flex items-center justify-between">
            ...
            <ExportMenu onExport={(format) => exportSongMetrics(metrics, format)} />
            </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <MetricCard
          ...
          icon={Play}
          description="All-time plays for this song"
        />
        <MetricCard
          title="Total Likes"
          value={metrics.totalLikes}
          metric={metrics.periodLikes}
          icon={Heart}
          description="All-time likes for this song"
        />
      </div>
    </div>
  )
